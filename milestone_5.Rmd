...
state_total_voting_age_pop <- age_characteristics_voting_2018_revised %>%
...
by_abbreviation_total_voting_age <- age_characteristics_voting_2018_revised %>%
  as.numeric(character) %>%
  ggplot(aes(state_abbreviation, 'Total Voting Age Population (estimate)')) + geom_bar(stat = "identity") +
  scale_y_continuous()
...
voting_by_age_sex_nov_2018 <- read_xlsx("Raw-data/voting_by_age_sex_nov_2018.xlsx") %>%
  clean_names()

voting_by_age_sex_nov_2018_revised <- voting_by_age_sex_nov_2018 %>%
  select(! "table_with_row_headers_in_columns_a_and_b_and_column_headers_in_rows_5_through_7") %>%
  rename(
    "Age" = x2,
    "Total Population" = x3,
    "Total Citizen Population" = x4,
...
voting_by_age_sex_nov_2018_revised %>%
  ggplot(aes(Age, 'Total Population')) + geom_bar(stat = "identity") +
  labs(
